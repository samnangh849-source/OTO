...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('message_updated', (update: { id: number, isReplied: boolean }) => {
    ...
    return () => {
      ...
      socket.off('message_media_ready');
  ...
  const confirmSend = async () => {
    ...
    try {
      ...
      if (pendingSend.type === 'automation') {
        const lastIncoming = messages.find(m => m.senderId === selectedChatId && !m.isOutgoing);
        if (!lastIncoming) throw new Error('No message to reply to');
        res = await api.post('/reply', {
          messageId: lastIncoming.id,
          telegramMessageId: lastIncoming.telegramMessageId,
  ...
  const activeChat = React.useMemo(() => 
    selectedChatId ? conversations.find(c => c.senderId === selectedChatId) : null
  , [selectedChatId, conversations]);

  const activeChatMessages = React.useMemo(() => 
    visibleMessages.filter(m => m.senderId === selectedChatId).sort((a,b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime())
  ...
  useEffect(() => {
    if (activeChatMessages.length > 0) scrollToBottom('smooth');
  ...
  useEffect(() => {
    setMessageRateSeconds(recentMessageTimestamps.length);
  }, [recentMessageTimestamps]);

  useEffect(() => {
    const counts: Record<string, number> = {};
    messages.forEach((m) => {
      if (!m.isOutgoing && !m.isReplied && m.accountId) {
  ...
  return (
    <div className="flex h-screen bg-binance-bg text-binance-text overflow-hidden font-sans">
      <div className="w-[320px] lg:w-[380px] bg-binance-panel border-r border-binance-border flex flex-col z-20">
        <div className="p-4 border-b border-binance-border">
          ...
          <div className="flex bg-binance-card p-1 rounded-md mb-4">
            <button onClick={() => setView('chat')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-xs font-semibold transition-all ${view === 'chat' ? 'bg-binance-panel text-binance-yellow shadow-sm' : 'text-binance-text-dim hover:text-binance-text'}`}><MessageSquare size={14} /> Chats</button>
            <button onClick={() => setView('stats')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-xs font-semibold transition-all ${view === 'stats' ? 'bg-binance-panel text-binance-yellow shadow-sm' : 'text-binance-text-dim hover:text-binance-text'}`}><BarChart3 size={14} /> Markets</button>
          </div>
          
          {view === 'chat' && (
            <div className="relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-binance-text-dim" size={16} />
              <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 bg-binance-bg border border-binance-border rounded-md focus:border-binance-yellow outline-none transition-all text-sm text-binance-text placeholder-binance-text-dim" />
            </div>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          {view === 'chat' ? (
            <motion.div key="chat-list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 overflow-y-auto">
              <div className="py-2">
                  ...
                  return (
                    <div key={chat.senderId} onClick={() => setSelectedChatId(chat.senderId)} className={`px-4 py-3 cursor-pointer transition-all flex items-center gap-3 relative group ${isActive ? 'bg-binance-card/80 border-l-4 border-binance-yellow' : 'hover:bg-binance-card/40 border-l-4 border-transparent'}`}>
                      <div className="relative flex-shrink-0">
                        {chat.senderPhoto && !failedImages[chat.senderId] ? (
                          <img src={chat.senderPhoto} alt={chat.senderName} className="w-12 h-12 rounded-full object-cover bg-binance-bg ring-1 ring-binance-border" onError={() => setFailedImages(prev => ({ ...prev, [chat.senderId]: true }))} />
                        ) : (
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-sm transition-colors ${isActive ? 'bg-binance-yellow text-[#181a20]' : 'bg-binance-panel text-binance-text-dim group-hover:bg-binance-card'}`}>{(chat.senderName || '?')[0]}</div>
                        )}
                        {userStatuses[chat.senderId] === 'online' && ( <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-binance-green border-2 border-binance-panel rounded-full shadow-sm" /> )}
                      </div>
                      <div className="flex-1 min-w-0 py-0.5">
                        <div className="flex justify-between items-start mb-1 gap-2">
                          <div className="min-w-0">
                            <span className={`text-sm font-semibold truncate ${isActive ? 'text-white' : 'text-binance-text'}`}>{chat.senderName}</span>
                          ...
                          <span className="text-[10px] text-binance-text-dim whitespace-nowrap">{format(new Date(chat.lastMessage.timestamp), 'dd/MM HH:mm')}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <div className="flex flex-col gap-0.5 max-w-[78%]">
                            <p className={`text-xs truncate ${isTyping ? 'text-binance-green font-medium animate-pulse' : 'text-binance-text-dim'}`}>
                              {isTyping ? 'typing...' : ( <>{chat.lastMessage.isOutgoing && <span className="text-binance-green/80 mr-1 text-[10px]">✓✓</span>}{chat.lastMessage.type === 'text' ? chat.lastMessage.text : `[${chat.lastMessage.type.toUpperCase()}]`}</> )}
                            </p>
                            <div className="flex items-center gap-2">
                              <p className={`text-[10px] font-semibold ${getUserStatusColor(chat.senderId)}`}>{getUserStatusText(chat.senderId)}</p>
                              {unreadCount > 0 && ( <span className="text-[8px] font-bold text-white bg-red-500 rounded-full px-2 py-0.5">{unreadCount}</span> )}
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            <motion.div key="stats-menu" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 space-y-4">
              <div className="rounded-md bg-binance-card border border-binance-border p-3">
                ...
                <div className="flex justify-between items-center text-sm mb-2"><span className="text-binance-text">Socket.IO</span><span className="text-binance-green font-bold">CONNECTED</span></div>
                <div className="flex justify-between items-center text-sm"><span className="text-binance-text">Telegram Auth</span><span className={`font-bold ${telegramStatus === 'connected' ? 'text-binance-green' : 'text-binance-yellow'}`}>{telegramStatus.toUpperCase()}</span></div>
              </div>
              {isGeneratingQr && ( <div className="flex flex-col items-center justify-center py-4 gap-2"><div className="w-6 h-6 border-2 border-binance-yellow/30 border-t-binance-yellow rounded-full animate-spin" /><span className="text-[10px] text-binance-text-dim italic font-medium">Generating Secure QR...</span></div> )}
              {qrCode && ( <div className="flex flex-col items-center gap-3 py-2 animate-in fade-in zoom-in duration-500"><div className="p-2 bg-white rounded-lg shadow-inner ring-4 ring-white/5"><img src={qrCode} alt="Telegram QR" className="w-32 h-32" /></div><p className="text-[10px] text-center text-binance-text font-medium bg-binance-yellow/10 px-3 py-1.5 rounded-full border border-binance-yellow/20">Scan with Telegram → Devices → Link Device</p><button onClick={() => { setQrCode(null); setIsGeneratingQr(false); }} className="text-[10px] text-binance-text-dim hover:text-white underline decoration-dotted underline-offset-4">Cancel</button></div> )}
              {qrPasswordRequired && ( <div className="flex flex-col gap-2 pt-2 border-t border-binance-border/30"><p className="text-[10px] text-binance-yellow font-bold">2-Step Verification Required</p><input type="password" placeholder="Enter 2FA Password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full px-3 py-2 bg-binance-panel border border-binance-yellow/30 rounded text-xs focus:border-binance-yellow outline-none" /><button onClick={submitPassword} className="w-full py-2 bg-binance-yellow text-[#181a20] rounded font-bold text-xs">Submit Password</button></div> )}
              {qrError && ( <div className="p-2 bg-red-500/10 border border-red-500/20 rounded text-[10px] text-red-400 font-medium">Error: {qrError}<button onClick={requestQr} className="ml-2 underline">Retry</button></div> )}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="flex-1 flex flex-col relative bg-binance-bg">
        {telegramStatus !== 'connected' && ( <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-md p-6 bg-binance-panel border border-binance-border rounded-xl shadow-xl text-center"><p className="text-sm font-semibold text-binance-text">Telegram disconnected: please relink in Settings</p><p className="text-xs text-binance-text-dim mt-2">Your messages are currently read-only until Telegram is connected.</p><div className="mt-4 flex flex-col gap-2"><button onClick={() => navigate('/settings')} className="w-full py-2 bg-binance-yellow text-[#181a20] rounded-md font-bold text-sm">Connect Telegram now</button><button onClick={() => socket.emit('check_telegram_status')} className="w-full py-2 bg-binance-card text-binance-text rounded-md border border-binance-border text-sm">Retry status</button></div></div></div> )}
        <AnimatePresence mode="wait">
          {view === 'stats' ? (
            <motion.div key="analytics" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 overflow-y-auto p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                <div><h2 className="text-xl font-bold text-binance-text">Market Overview</h2><p className="text-xs text-binance-text-dim">Real-time incoming + outgoing counters (last 60s)</p></div>
                <div className="flex items-center gap-3"><div className="flex items-center gap-2 px-3 py-1 bg-binance-green/10 rounded-full border border-binance-green/20"><div className="w-1.5 h-1.5 rounded-full bg-binance-green animate-pulse shadow-[0_0_8px_rgba(14,203,129,0.8)]" /><span className="text-[10px] font-black text-binance-green uppercase tracking-widest">Reel Time Live</span></div><div className="flex items-center gap-2 px-3 py-1 bg-binance-panel rounded-full border border-binance-border"><div className={`w-2 h-2 rounded-full ${getTrafficColor(messageRateSeconds)}`} /><span className="text-[10px] font-bold text-binance-yellow">{messageRateSeconds}</span><span className="text-[10px] ml-1 text-binance-text-dim">/60s</span></div></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[ { label: 'Total Messages', value: stats?.totalMessages, icon: MessageSquare, color: 'text-binance-text' }, { label: 'Active Users', value: stats?.totalUsers, icon: Users, color: 'text-binance-text' }, { label: 'Fill Rate (%)', value: stats?.totalMessages ? Math.round((stats.repliedMessages / stats.totalMessages) * 100) + '%' : '0%', icon: CheckCircle2, color: 'text-binance-green' }, { label: 'Avg Latency', value: '~4m', icon: Clock, color: 'text-binance-yellow' } ].map((stat, i) => (
                  <motion.div key={i} initial={false} animate={{ scale: [1, 1.02, 1] }} transition={{ duration: 0.3 }} className="bg-binance-panel p-5 rounded-md border border-binance-border hover:border-binance-text-dim transition-colors group relative overflow-hidden"><p className="text-xs font-medium text-binance-text-dim mb-2">{stat.label}</p><div className="flex items-end justify-between relative z-10"><h3 className={`text-2xl font-bold ${stat.color} tabular-nums`}>{stat.value || 0}</h3><stat.icon size={20} className="text-binance-text-dim opacity-30 group-hover:opacity-50 transition-opacity" /></div></motion.div>
                ))}
              </div>
            </motion.div>
          ) : (
            <motion.div key="chat-active" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col h-full relative">
              {activeChat ? (
                <>
                  <div className="px-6 py-4 border-b border-binance-border flex items-center justify-between bg-binance-panel z-20">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        {activeChat.senderPhoto && !failedImages[activeChat.senderId] ? ( <img src={activeChat.senderPhoto} alt={activeChat.senderName} className="w-10 h-10 rounded-full object-cover" /> ) : ( <div className="w-10 h-10 rounded-full bg-binance-card text-binance-yellow flex items-center justify-center font-bold text-lg">{(activeChat.senderName || '?')[0]}</div> )}
                        {userStatuses[activeChat.senderId] === 'online' && ( <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-binance-green border-2 border-binance-panel rounded-full" /> )}
                      </div>
                      <div><h3 className="font-bold text-binance-text text-lg leading-tight">{activeChat.senderName}</h3><div className="text-xs flex items-center gap-1.5 mt-0.5"><span className={`font-bold ${getUserStatusColor(activeChat.senderId)} ${typingUsers[activeChat.senderId] ? 'animate-pulse' : ''}`}>{getUserStatusText(activeChat.senderId)}</span></div></div>
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-1 bg-binance-bg relative">
                    ...
                    {activeChatMessages.map((msg, i) => {
                      ...
                      const nextMsg = activeChatMessages[i + 1], prevMsg = activeChatMessages[i - 1];
                      const isLastInGroup = !nextMsg || !!nextMsg.isOutgoing !== isMine, isFirstInGroup = !prevMsg || !!prevMsg.isOutgoing !== isMine;
                      const showAvatar = !isMine && isLastInGroup;
                      return (
                        <div key={msg.id} className={`flex items-end gap-2 mb-1 ${isMine ? 'flex-row-reverse' : 'flex-row'} ${isLastInGroup ? 'mb-4' : 'mb-0.5'}`}>
                          {!isMine && ( <div className="w-8 flex-shrink-0">{showAvatar ? ( <img src={msg.senderPhoto || `https://ui-avatars.com/api/?name=${msg.senderName}&background=1e2329&color=848e9c`} className="w-8 h-8 rounded-full object-cover shadow-sm ring-1 ring-white/10" alt="" /> ) : null}</div> )}
                          <div className={`message-bubble max-w-[80%] sm:max-w-[70%] px-4 py-2.5 text-sm relative z-10 backdrop-blur-xl border transition-all ${isMine ? `bg-[#0088cc]/60 border-white/20 text-white shadow-[0_8px_32px_0_rgba(0,136,204,0.2)] ${isLastInGroup ? 'rounded-2xl rounded-br-none message-bubble-tail-out' : 'rounded-2xl'}` : `bg-white/5 border-white/10 text-white/90 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] ${isLastInGroup ? 'rounded-2xl rounded-bl-none message-bubble-tail-in' : 'rounded-2xl'}`}`}>
                            {isFirstInGroup && !isMine && ( <span className="text-[11px] font-bold text-[#0088cc] mb-0.5">{msg.senderName}</span> )}
                            {msg.type === 'text' && <p className="leading-relaxed whitespace-pre-wrap">{msg.text}</p>}
                            {msg.type === 'image' && ( <div className="relative group overflow-hidden rounded-lg"><img src={msg.text} className="max-h-72 w-full object-contain bg-black/20" alt="media" /><div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors pointer-events-none" /></div> )}
                            {msg.type === 'voice' && ( <audio controls preload="metadata" className="max-w-[240px]"><source src={msg.text} type="audio/ogg" /><source src={msg.text} type="audio/mpeg" /></audio> )}
                            {msg.type === 'video' && ( <video controls preload="metadata" className="max-h-72 w-full rounded-lg bg-black/20"><source src={msg.text} type="video/mp4" /></video> )}
                            <div className={`text-[9px] self-end font-semibold flex items-center gap-1.5 mt-0.5 ${isMine ? 'text-[#181a20]/70' : 'text-binance-text-dim'}`}>{format(new Date(msg.timestamp), 'dd/MM HH:mm')}{isMine && ( <span className="flex items-center">{(msg.telegramMessageId && lastReadIds[activeChat.senderId] >= msg.telegramMessageId) ? <span className="text-binance-green text-[10px] drop-shadow-sm">✓✓</span> : <span className="text-[10px]">✓</span>}</span> )}</div>
                          </div>
                        </div>
                      );
                    })}
      ...
      <AnimatePresence>
        {showPreview && pendingSend && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-binance-panel rounded-lg shadow-2xl w-full max-w-lg overflow-hidden border border-binance-border">
              <div className="p-4 border-b border-binance-border flex justify-between items-center bg-binance-card"><h4 className="text-base font-bold text-binance-text flex items-center gap-2"><Send size={16} className="text-binance-yellow" /> Confirm Order</h4><button onClick={() => { setShowPreview(false); setPendingSend(null); }} className="text-binance-text-dim hover:text-binance-text"><X size={20} /></button></div>
